...
if string_length(string_digits(m2_ipv_time)) > 0 && string_length(string_digits(m2_ipv_time)) < 5    
{
    temp_array[0] = 1;
    {
...
if m2_ipv_name != ""   temp_array[1] = 1;

if m2_ipv_year != ""   temp_array[2] = 1;
if real(m2_ipv_month) > 0 && real(m2_ipv_month) < 13   temp_array[3] = 1;
if real(m2_ipv_day) > 0   && real(m2_ipv_day) < (scr_call_daysinthemonth(real(m2_ipv_month),0) + 1)    temp_array[4] = 1;
if real(m2_ipv_hour) >= 0   && real(m2_ipv_hour) < 24    temp_array[5] = 1;
if real(m2_ipv_minute) >= 0   && real(m2_ipv_minute) < 60    temp_array[6] = 1;

if temp_array[3] = 1
{
    e_day = "#Please insert a valid day.(1,2,3..."+string(scr_call_daysinthemonth(real(m2_ipv_month),0))+")";
}
...
for (i=0;i<=6;i+=1)
{
    if temp_array[i] == 0
    {
        ...
        if i == 4    {if temp_array[3] == 0{     if !real(ipv_day) > 0   or !real(m2_ipv_day) < 32    error_message += e_day;} else if  temp_array[3] == 1{   error_message += e_day;}}
